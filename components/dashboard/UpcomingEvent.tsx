import React, { useCallback, useMemo } from "react";
import { View, Text, Image, ActivityIndicator } from "react-native";
import { router, Href } from "expo-router";
import { UpcomingEvent as UpcomingEventType, User } from "./types";
import { formatEventDate } from "./utils";
import ActionButtonsCarousel from "./ActionButtonsCarousel";
import { LinearGradient } from "expo-linear-gradient";
import Colors from "@/constants/Colors";

interface UpcomingEventProps {
  upcomingEvent: UpcomingEventType;
  user: User;
}

const UpcomingEvent: React.FC<UpcomingEventProps> = React.memo(
  ({ upcomingEvent, user }) => {
    const formattedDate = useMemo(
      () => formatEventDate(upcomingEvent.start_at),
      [upcomingEvent.start_at]
    );

    const getAvailableCount = useCallback(
      (items: any[] | null) => {
        return (
          items
            ?.filter((item) => item.event.id === upcomingEvent.id)
            ?.filter((item) => !item.is_validated)?.length ?? 0
        );
      },
      [upcomingEvent.id]
    );
    const getAvailableCourtesyCount = useCallback(
      (items: any[] | null) => {
        return (
          items
            ?.filter((item) => item.id === upcomingEvent.id)
            ?.filter((item) => !item.courtesy.is_validated)?.length ?? 0
        );
      },
      [upcomingEvent.id]
    );

    const availableTickets = useMemo(
      () => getAvailableCount(user?.tickets || []),
      [getAvailableCount, user.tickets]
    );
    const availableDrinks = useMemo(
      () => getAvailableCount(user?.drinks || []),
      [getAvailableCount, user.drinks]
    );
    const availableCourtesies = useMemo(
      () => getAvailableCourtesyCount(user?.courtesies || []),
      [getAvailableCourtesyCount, user.courtesies]
    );

    const handleTicketsPress = useCallback(() => {
      const url = `/(dashboard)/events/${upcomingEvent.id}`;
      router.push(url as Href);
    }, [upcomingEvent.id]);

    const handleDrinksPress = useCallback(() => {
      const url = `/(dashboard)/events/${upcomingEvent.id}/drinks`;
      router.push(url as Href);
    }, [upcomingEvent.id]);

    const handleCourtesiesPress = useCallback(() => {
      const url = `/(dashboard)/events/${upcomingEvent.id}/courtesy`;
      router.push(url as Href);
    }, [upcomingEvent.id]);

    const eventAddress = useMemo(() => {
      return upcomingEvent.place;
    }, [upcomingEvent.place]);

    const actionButtons = useMemo(
      () => [
        {
          icon: require("../../assets/images/ticket.png"),
          title: "Entradas",
          count: availableTickets,
          onPress: handleTicketsPress,
          width: 32,
          height: 32,
        },
        {
          icon: require("../../assets/images/glass.png"),
          title: "Barra",
          count: availableDrinks,
          onPress: handleDrinksPress,
          width: 32,
          height: 32,
        },

        {
          icon: require("../../assets/images/gift.png"),
          title: "Cortesías",
          count: availableCourtesies,
          onPress: handleCourtesiesPress,
          width: 48,
          height: 32,
        },
      ],
      [
        availableTickets,
        availableDrinks,
        availableCourtesies,
        handleTicketsPress,
        handleDrinksPress,
        handleCourtesiesPress,
      ]
    );

    // Early return if event is not available
    if (!upcomingEvent) {
      return (
        <View className="flex mx-2">
          <View className="mb-4">
            <Text className="text-white font-bold text-xl mx-2">
              Tu próximo evento
            </Text>
          </View>
          <View className="bg-white rounded-xl mx-2 mb-4 p-4">
            <View className="flex items-center justify-center py-8">
              <ActivityIndicator size="small" color="#666" />
              <Text className="text-secondary-300 text-sm mt-2">
                Cargando evento...
              </Text>
            </View>
          </View>
        </View>
      );
    }

    return (
      <View className="flex">
        <View className="mb-4">
          <Text className="text-white font-bold text-xl mx-2">
            Tu próximo evento
          </Text>
        </View>
        <View className="border border-white/20 rounded-xl mx-2 mb-4">
          <View className="flex pb-0 flex-col rounded-xl relative">
            <View className="flex w-full">
              <Image
                source={{ uri: upcomingEvent.image }}
                className="rounded-xl w-full h-[160px]"
                resizeMode="cover"
              />
            </View>
            <LinearGradient
              colors={["transparent", Colors.secondary[500]]} // Example: dark to transparent
              className="absolute bottom-0 left-0 right-0 py-4 px-4"
            >
              <View className="flex flex-col w-3/4 px-2">
                <Text
                  numberOfLines={1}
                  className="text-white overflow-hidden font-bold text-lg"
                >
                  {upcomingEvent.name}
                </Text>
                <Text className="text-primary-500 text-base font-semibold">
                  {formattedDate}
                </Text>
                <View className="flex flex-row items-center gap-2">
                  <Text
                    numberOfLines={1}
                    className="text-secondary-100 text-sm"
                  >
                    {eventAddress}
                  </Text>
                </View>
              </View>
            </LinearGradient>
          </View>

          <ActionButtonsCarousel buttons={actionButtons} />
        </View>
      </View>
    );
  }
);

export default UpcomingEvent;
